...
fn conf_mayastor() {
    // configuration yaml does not yet support replicas to configure the
...
fn replica_snapshot() {
    ...
    Reactor::block_on(async {
        create_nexus().await;
        write_some().await;
        invalid_nvme_admin(0xc1).await.unwrap();
...
async fn invalid_nvme_admin(opc: u8) -> Result<(), CoreError> {
    let h = BdevHandle::open(NXNAME, true, false).unwrap();
    h.nvme_admin_custom(opc)
        .await
