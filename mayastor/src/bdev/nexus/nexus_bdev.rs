...
extern "C" fn nvmf_create_snapshot_hdlr(req: *mut spdk_nvmf_request) -> i32 {
    ...
    let _cmd = unsafe { spdk_sys::spdk_nvmf_request_get_cmd(req) };
    ...
    if let Some(replica) = Replica::from_bdev(&bd) {
        let mut data: *mut nix::libc::c_void = std::ptr::null_mut();
        let mut length: u32 = 0;
        unsafe {
            spdk_sys::spdk_nvmf_request_get_data(req, &mut data, &mut length)
        };
        ...
        debug!(
            "nvmf_request length {}, snapshot_name {}",
            length, snapshot_name
        );
        // FIXME handle async response
        let ret = match replica.create_snapshot(req, "snapshot_name") {
            Ok(_) => 1, // SPDK_NVMF_REQUEST_EXEC_STATUS_ASYNCHRONOUS
            Err(_e) => -1,
        };
        debug!("hdlr ret {}", ret);
        ret
...
pub fn setup_create_snapshot_hdlr() {
    ...
    debug!("nvmf_set_custom_admin_cmd_hdlr for create_snapshot_hdlr");
}
...
impl Nexus {
    ...
    pub(crate) fn nvme_admin(
        ...
        debug!("nvme_admin buf {:?}", io.nvme_buf());
        debug!("nvme_admin nbytes {}", io.nvme_nbytes());
